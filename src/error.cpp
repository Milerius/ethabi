...
namespace ethabi::error
{
  ...
  std::string param_type_read_category::message(int error_value) const
  {
    switch (static_cast<param_type_read>(error_value))
    {
      case param_type_read::invalid_name: return "invalid_name";
      default: return "(unrecognized error)";
    }
  }

  const char* param_type_read_category::name() const noexcept
  {
    return "param_type";
  }
