...
{
    [[maybe_unused]] read_result_t
    read([[maybe_unused]] const std::string& name)
    {
            ...
            for (auto&& [pos, c]: ranges::views::enumerate(name))
            {
                ...
                case parser_token::right_parenthesis:
                {
                    ...
                    else if (nested > 0)
                    {
                        ...
                        auto inner_tuple = std::string(begin(name) + static_cast<long>(top_level_paren_open), begin(name) + static_cast<long>(pos) + 1);
